...
1. **`ONSITE.md`** (repo root) — single-machine setup: Python version
   ...
   (WCH driver), running `games\test_hardware.py`, starting the API with
   `USE_SERIAL_HD=1`. Follow every step in order; do not skip verification
   steps.
...
1. Follow `ONSITE.md` steps 1–8 to get Python installed, dependencies
   installed, the hardware shelve verified, the COM port confirmed visible
   in Device Manager, and the API server started with `USE_SERIAL_HD=1`.
...
3. Execute the **on-site validation checklist** in `HARDWARE_VALIDATION.md`
   §3, in order. This includes:
   - Confirming `USE_SERIAL_HD=1` boots cleanly (hardware init log line, not
     `Hardware init failed`).
   - Running `games\test_hardware.py` and confirming green floor + sensor
     reads work.
   - Verifying **each of the 6 hoop positions individually** — pay specific
     attention to hoop/column index 5 (the 6th hoop), a known trouble spot
     where levels authored for 5 columns may leave it dark during real
     gameplay even though the diagnostic script proves it works
     electrically.
...
checklist item in `HARDWARE_VALIDATION.md` §3, report explicit **PASS** or
**FAIL** with a one-line observation, e.g.:

```
[PASS] USE_SERIAL_HD=1 boot — log showed "Hardware ready: 1 port(s), 1x6, layout=0"
[FAIL] Hoop column 5 — lit correctly in test_hardware.py but stayed dark during
       real gameplay on levels 1-3; matches known 5-col-level/6-col-floor gap
